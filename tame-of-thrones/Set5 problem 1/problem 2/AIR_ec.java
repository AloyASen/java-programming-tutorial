...
public class AIR_ec implements ElectionCouncil {
    ...
    public void getMessages(Message message) {
        //if  the receiver is not a competent
        if(! isCompetent()){

            //check the message if it contains the emblem
            boolean checkMessage = checkMessage(message.getMsg().toLowerCase());
            if(checkMessage){
                //increase vote count if the sender is not competant
    ...
    private boolean checkMessage(String msg) {
        char[] EmblemArray = AIR.emblem.toCharArray();
        /**
         * this is a brute force technique and will be improved if required
         */

        for (char emlm : EmblemArray){
            if(containsChar(msg, emlm) && (occurances(msg, emlm, 0) >= occurances(AIR.emblem, emlm, 0))){
